...
function image_upload(field, thumb) {
    ...
    $('#dialog').dialog({
        ...
        close: function (event, ui) {
            if ($('#' + field).attr('value')) {
                $.ajax({
                    url: BASE_URL + 'index.php?route=common/filemanager/image&token=<?php echo $token; ?>&image=' + encodeURIComponent($('#' + field).val()),
                    dataType: 'text',
                    success: function(data) {
                        if (data != "") {
...
$('.browseNode').bind('click', function(e){
  ...
  $.ajax({
      url: BASE_URL + 'index.php?route=openbay/amazonus_listing/getBrowseNodes&token=<?php echo $token; ?>',
      type: 'POST',
      dataType: 'json',
      beforeSend: function(){
