...
<script type="text/javascript"><!--
  ...
  function load(){
        $.ajax({
            url: BASE_URL + 'index.php?route=openbay/ebay/editLoad&token=<?php echo $token; ?>&product_id=<?php echo $product_id; ?>',
            type: 'GET',
            dataType: 'json',
            beforeSend: function(){
  ...
  $('#button-save').bind('click', function() {
    $.ajax({
      type: 'POST',
      url: BASE_URL + 'index.php?route=openbay/ebay/editSave&token=<?php echo $token; ?>',
      dataType: 'json',
      data: $("#form").serialize(),
      beforeSend: function(){
  ...
  $('#button-remove-link').on('click', function () {
    ...
    if (pass == true) {
      ...
      if (id !== '') {
        $.ajax({
          type: 'GET',
          url: BASE_URL + 'index.php?route=openbay/ebay/removeItemLink&token=<?php echo $token; ?>&product_id=<?php echo $product_id; ?>',
          dataType: 'json',
          success: function () {
            alert('<?php echo $text_alert_removed; ?>');
            window.location = BASE_URL + 'index.php?route=extension/openbay/items&token=<?php echo $token; ?>';
          },
          error: function (xhr, ajaxOptions, thrownError) {
            if (xhr.status != 0) {
              alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
  ...
  $('#button-end-item').on('click', function () {
    ...
    if (pass == true) {
      ...
      if (id !== '') {
        $.ajax({
          type: 'GET',
          url: BASE_URL + 'index.php?route=openbay/ebay/endItem&token=<?php echo $token; ?>&id=' + id,
          dataType: 'json',
          success: function (data) {
            if (data.error == true) {
              alert(data.msg);
