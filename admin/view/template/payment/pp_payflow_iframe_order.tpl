...
function doVoid() {
    if (confirm('<?php echo $text_confirm_void; ?>')) {
        $.ajax({
            ...
            url: BASE_URL + 'index.php?route=payment/pp_payflow_iframe/void&token=<?php echo $token; ?>',
            beforeSend: function() {
...
function capture() {
    ...
    $.ajax({
        ...
        url: BASE_URL + 'index.php?route=payment/pp_payflow_iframe/capture&token=<?php echo $token; ?>',
