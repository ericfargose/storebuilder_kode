...
require_once(DIR_SYSTEM . 'library/url_helper/UrlMode.php');
...
$urlMode = UrlModeFactory::getInstance($registry, array(
   'HTTP_HOST' => strtolower($_SERVER['HTTP_HOST']),
   'REQUEST_URI' => strtolower($_SERVER['REQUEST_URI']),
));

$registry->set('url_mode', $urlMode);

$sql = "SELECT ss.*, sxs.status FROM " . DB_PREFIX . "sb_store ss LEFT JOIN " . DB_PREFIX . "sb_xstore_meta sxs ON(sxs.store_id = ss.store_id) WHERE url IN ('" . implode("', '", $urlMode->getMatchUrls()) . "') ";
$store_query = $db->query($sql); 
if($store_query->num_rows > 0){
	if($store_query->row['store_id'] == 1) {
		header('Location: create/index.php');
		exit;
	}
	if($store_query->row['status'] == 'active') {
	} elseif($store_query->row['status'] == 'disabled') {
		header("HTTP/1.0 404 Not Found");
		echo file_get_contents('store_inactive.html');
		exit;
	}
} else {
	header("HTTP/1.0 404 Not Found");
	echo file_get_contents('store_not_found.html');
	exit;
}
$db->forceClose();

$db = new DB(DB_DRIVER, DB_HOSTNAME, DB_USERNAME, DB_PASSWORD, $store_query->row['db_name']);
